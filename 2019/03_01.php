...
function calc_smallest_distance($wires) {
    $field = create_field($wires);
    $field = draw_wire($field,$wires['wire1'], 1);
    $field = draw_wire($field,$wires['wire2'], 2);
    #draw_field($field);
    $crosses = find_crosses($field);
    $distances = array_map('calculate_cross_distance', $crosses);
    $min = min($distances);
    return $min;
}

foreach($test_cases as $test) {
    if($test['distance'] != calc_smallest_distance($test)) {
        echo "FAILED\n";
    }
}

echo calc_smallest_distance($input)."\n";
