...
function determine_combination($input_code) {
    $max = count($input_code);
    for($i=0;$i<$max;$i++) {
        for($j=0;$j<$max;$j++) {
            echo '.';
            $temp = $input_code;
...
list ($noun, $verb) = determine_combination($input_code);

$result = 100 * $noun + $verb;

echo "result value Noun: {$noun}, Verb: {$verb}, Result {$result}\n";
