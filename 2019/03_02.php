...
function draw_field($field) {
    $draw_field = array_reverse($field);
    foreach($draw_field as $row) {
...
function create_field($wires) {
    $field_dims = determine_field_size($wires);
    $field = [];
    for($y = $field_dims['y_min']; $y<$field_dims['y_max'];$y++){
        $field[$y] = array_fill($field_dims['x_min'],$field_dims['x_max'],0);
    }
    return $field;
}

function determine_field_size($wires) {
    $dims1 = get_wire_dims($wires['wire1']);
    $dims2 = get_wire_dims($wires['wire2']);

    $dims = [];

    $dims['x_max'] = (max($dims1['x_max'], $dims2['x_max'])) + 3;
    $dims['y_max'] = (max($dims1['y_max'], $dims2['y_max'])) + 3;

    $dims['x_min'] = (min($dims1['x_min'], $dims2['x_min'])) - 1;
    $dims['y_min'] = (min($dims1['y_min'], $dims2['y_max'])) - 1;
    return $dims;
}

function get_wire_dims($wire) {
    $wire_steps = parse_wire($wire);
    $dims = ['x_max' => 0, 'y_max' => 0,'x_min' => 0, 'y_min' => 0,];
    $pos  = ['x' => 0, 'y' => 0];
    foreach($wire_steps as $step) {
        ...
        $dims['x_max'] = max($dims['x_max'], $pos['x']);
        $dims['y_max'] = max($dims['y_max'], $pos['y']);

        $dims['x_min'] = min($dims['x_min'], $pos['x']);
        $dims['y_min'] = min($dims['y_min'], $pos['y']);
    }
    return $dims;
}
...
function parse_wire($wire) {
    $temp = explode(',',$wire);
    return array_map('map_wire', $temp);
}

function map_wire($item) {
    $return = [];
    $return['direction'] = substr($item,0, 1);
    if(!in_array($return['direction'], ['D','U','L','R'])) {
        die("INVALID DIRECTION\n");
    }
    $return['length'] = intval(substr($item, 1));
    return $return;
}

function draw_wire($field, $wire, $wire_number) {
    $wire_steps = parse_wire($wire);
    ...
    foreach($wire_steps as $step) {
        ...
        switch($step['direction']) {
            case 'U':
                for($y = ($pos['y']+1); $y <= ($pos['y'] + $step['length']); $y++) {
                    $field[$y][$pos['x']] += $wire_number;
                }
                $pos['y'] += $step['length'];
                break;
            case 'D':
                for($y = ($pos['y'] - 1); $y > ($pos['y'] - $step['length'] - 1); $y--) {
                    $field[$y][$pos['x']] += $wire_number;
                }
                $pos['y'] -= $step['length'];
                break;
            case 'R':
                for($x = ($pos['x'] + 1 ); $x <= ($pos['x'] + $step['length']); $x++) {
                    $field[$pos['y']][$x] += $wire_number;
                }
                $pos['x'] += $step['length'];
                break;
            case 'L':
                for($x = ($pos['x']-1); $x > ($pos['x'] - $step['length'] -1); $x--) {
                    $field[$pos['y']][$x] += $wire_number;
                }
                $pos['x'] -= $step['length'];
                break;
...
function calculate_cross_distance($item) {
    return abs($item['x']) + abs($item['y']);
}

function calc_fewest_steps($wires) {
    $field = create_field($wires);
    $field = draw_wire($field,$wires['wire1'], 1);
    $field = draw_wire($field,$wires['wire2'], 2);
    #draw_field($field);
    $crosses = find_crosses($field);

    $steps = [];
    foreach($crosses as $cross) {
        $steps[] = calculate_steps_sum($cross, $wires);
    }
    $min = min($steps);
    return $min;
}

function calculate_steps_sum($cross, $wires) {
    $steps_wire1 = calculate_steps($cross, $wires['wire1']);
    $steps_wire2 = calculate_steps($cross, $wires['wire2']);
    return $steps_wire1 + $steps_wire2;
}

function calculate_steps($cross, $wire) {
    $wire_steps = parse_wire($wire);
    $pos  = ['x' => 0, 'y' => 0];
    $steps_number = 0;
    foreach($wire_steps as $step) {
        ...
        switch($step['direction']) {
            case 'U':
                for($y = ($pos['y']+1); $y <= ($pos['y'] + $step['length']); $y++) {
                    if(($cross['y'] == $y) && ($cross['x'] == $pos['x'])) {
                        $length = ($y - $pos['y']);
                        echo $length . " ";
                        $steps_number += $length;
                        echo 'crossed UPWARDS'."\n";
                        break 3;
                    }
                }
                $pos['y'] += $step['length'];
        ...
        echo $step['length'] . ' + ';
        $steps_number += $step['length'];
    }
    return $steps_number;
}

foreach($test_cases as $test) {
    $calc_steps = calc_fewest_steps($test);
    var_dump($calc_steps);
    if($test['distance'] != $calc_steps) {
        var_dump($test['distance']);
        echo "FAILED\n";
    }
    echo "foo\n";
    #break;
